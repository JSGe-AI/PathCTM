...
class PatchCTM(nn.Module):
    ...
    @staticmethod
    def _tracking_payload(tracked_synch_out, tracked_synch_action, tracked_pre, tracked_post, tracked_attention):
        return (
            np.array(tracked_synch_out),
            np.array(tracked_synch_action),
            np.array(tracked_pre),
            np.array(tracked_post),
            np.array(tracked_attention),
        )

    def forward(self, x, indexs, relations, device, threshold=None, lazy=False, track=False, return_all_steps=False):
        batch_size = 1
        topk = 10
        patch_num1 = 0
        patch_num2 = 0
        ...
        for stepi in range(self.iterations):
            ...
            if not return_all_steps and threshold is not None and mean_certainty >= threshold:
                if track:
                    return (
                        current_prediction,
                        current_certainty,
        ...
        if return_all_steps:
            patch_breakdown = {
                "patch_num_8192": int(patch_num1),
                "patch_num_4096": int(patch_num2),
                "patch_num_2048": int(patch_num3),
                "patch_num_1024": int(patch_num4),
                "full_patch_num": int(patch_num),
                "standard_patch_num": int(patch_num_1024),
            }
            if track:
                return (
                    predictions,
            ...
            return predictions, certainties, patch_breakdown
